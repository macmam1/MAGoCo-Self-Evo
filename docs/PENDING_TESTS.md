...
| T14 | Capability gate + deferred queue | `POST /memory/gate-check` با تسک پیچیده روی مدل ضعیف → queued → `GET /memory/deferred` → resolve؛ + تست kill-switch (`enabled=false`) | حداقل | 🟡 نیمه: gate-check/compensate PASS؛ resolve نامعتبر ۴۰۰ تمیز؛ صف واقعی (queued→resolve) مانده |
